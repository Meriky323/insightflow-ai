...
def write_docx(research: dict, summary: dict) -> Path:
    ...
    c=summary.get('evidence_confidence') or {};doc.add_heading('Evidence quality',1);doc.add_paragraph(f"Confidence: {c.get('label','—')} (exploratory heuristic). Consumer evidence: {summary['review_count']}; product signals: {summary['product_count']}.")
    ...
    for x in summary.get('opportunities',[])[:5]:
        d=x.get('decision') or {};doc.add_paragraph(f"{x['name']} — validation hypothesis; {x['confidence']} confidence",style='List Bullet')
...
def write_pdf(research: dict, summary: dict) -> Path:
    ...
    meta=[[Paragraph('<b>Research</b><br/>'+_clean(research.get('keyword')),styles['body']),Paragraph('<b>Markets</b><br/>'+_clean(', '.join(rr.get('markets') or [])),styles['body']),Paragraph('<b>Evidence confidence</b><br/>'+_clean(f"{conf.get('label','—')} · exploratory"),styles['body'])]]
    ...
    for i,x in enumerate(decision_actions[:3],1):
        d=x.get('decision') or {};blocks=[Paragraph(f"0{i} · {_clean(x.get('name'))}",styles['h2']),Paragraph(f"Validation hypothesis · {_clean(x.get('confidence'))} evidence strength · not a market-size forecast",styles['small'])]
    ...
    story += [Paragraph('<b>AI boundary:</b> The public case uses saved analysis and deterministic retrieval, with no live LLM calls. Local analysis mode is recorded per research; a failed LLM call falls back to rules.',styles['body']),Spacer(1,3*mm)]
