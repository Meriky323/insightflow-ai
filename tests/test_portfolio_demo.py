...
def test_demo_is_traceable_and_market_guardrail_blocks_bad_comparison():
    ...
    assert s['research']['decision']['demo_meta']['demo_schema_version']=='1.5'
...
def test_public_demo_ask_is_local_and_grounded(monkeypatch):
    ...
    r=client.post(f'/api/research/{rid}/ask',json={'question':'Can we safely compare US and AU consumer preferences?'})
    assert r.status_code==200
...
def test_target_company_case_surface():
    ...
    assert 'workflow becomes the GTM battleground' in text
